...
export default function ClipboardScreen({ language, userId, refreshKey, triggerRefresh }) {
  ...
  const lastCapturedValueRef = useRef(null); // Anti-spam: track last captured value
  ...
  const startClipboardMonitoring = () => {
    stopClipboardMonitoring();
    // Check every 3 seconds - each read with content = new entry
  ...
  // ANTI-SPAM: Only create entry when clipboard VALUE CHANGES
  ...
  const captureClipboardEntry = async () => {
    ...
    try {
      ...
      // ANTI-SPAM: Skip if same as last captured value
      if (trimmedValue === lastCapturedValueRef.current) return;
      
      console.log('[Smart Clipboard] New value detected:', trimmedValue.substring(0, 30) + '...');
      
      // Update last captured value BEFORE saving
      lastCapturedValueRef.current = trimmedValue;
      
      await saveClipboardEntry(trimmedValue);
  ...
  // Save entry unconditionally - no deduplication at capture level
  const saveClipboardEntry = async (content) => {
    ...
    try {
      ...
      console.log('[Smart Clipboard] Saving:', entryId);
  ...
  const activateSmartClipboard = async () => {
    // Reset last captured value - will capture on first clipboard change
    ...
    console.log('[Smart Clipboard] Activated - waiting for clipboard changes');
  };
